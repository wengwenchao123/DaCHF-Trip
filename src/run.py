...
def main(opt):
    ...
    for split_index in tqdm(range(df.shape[0])):
        ...
        trainer = ARTrainer(train_dataset=train_dataset, eval_dataset=test_dataset, opt=opt, poi_dis_dict=poi_dis_dict,
                            lr=opt.lr, confidence_score=opt.confidence,data=opt.dataset,
                            decode_type=opt.decoding_type, train_type=opt.training_type, batch_size=opt.batch_size,
                            ...
                            adjacent_matrix=train_am, position_matrix=train_pm,device = opt.device)
...
if __name__ == '__main__':
    ...
    # strategy
    parser.add_argument('--decoding_type', type=str, default='Top-NP',
    ...
    # ablation study
    parser.add_argument('--contrast', action='store_true', default=True, help='ablation: using contrast')
