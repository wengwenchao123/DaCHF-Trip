...
class Time2Vec(nn.Module):
    ...
    def forward(self, x):
        ...
        new_x = torch.zeros([x.shape[0], x.shape[1], self.out_dim],device=x.device)
...
class Recommender(nn.Module):
    def __init__(self, out_dim, poi_size):
        ...
        self.fc1 = nn.Linear(out_dim, 2)
        self.fc2 = nn.Linear(2, poi_size)
        self.leaky_relu = nn.LeakyReLU(0.2)

    def forward(self, outputs):
        x1 = self.fc1(outputs)
        x2 = self.fc2(x1)
        x = self.leaky_relu(x2)
        return x,x2


# Construct total framework(AR-Trip)
class HyperTrip(nn.Module):
    def __init__(self, venue_size, hour_size,opt,
                 max_length_venue_id=100, d_model=128, n_head=4, num_encoder_layers=1):

        super(HyperTrip, self).__init__()
        ...
        self.contrast_fn1 = Contrast(d_model, tau=0.3, lambda_sym=0.11)
        self.contrast_fn2 = Contrast(d_model, tau=0.3, lambda_sym=0.11)
        self.contrast= opt.contrast
    def forward(self, poi_input, hour_input, am, pm, src_mask, batch_num):

        hour_input = torch.gather(torch.linspace(start=0, end=1, steps=self.hour_size,device=poi_input.device),
        ...
        merge_embedded = self.fus(hour_embedded, poi_embedded, batch_size, seq_length, batch_num)
        ...
        # ssl_loss=0
        # guiding mechanism
        poi_output,embedding = self.predictor(model_output)  # [b,l,v]
        # drifting mechanism
        if self.contrast:
            ssl_loss = self.contrast_fn1(position_embedded, poi_embedded) + self.contrast_fn2(hour_embedded,
                                                                                              poi_embedded)
            # ssl_loss =  self.contrast_fn(poi_embedded, hour_embedded)

            penalty_loss = 0.12*ssl_loss
        else:
            penalty_loss = 0

        return poi_output, penalty_loss,embedding
